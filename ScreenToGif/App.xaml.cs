    ...
    public partial class App
    {
            ...
            if (UserSettings.All.SingleInstance)
            {
                try
                {
                    using (var thisProcess = Process.GetCurrentProcess())
                    {
                        ...
                        var name = thisProcess.MainModule?.FileName ?? Assembly.GetEntryAssembly()?.Location ?? "UxtrataRecorder";
        ...
        internal void ShowException(Exception exception)
        {
            if (Global.IsHotFix4055002Installed && exception is XamlParseException && exception.InnerException is TargetInvocationException)
                ExceptionDialog.Ok(exception, "UxtrataRecorder", "Error while rendering visuals", exception.Message);
            else
                ExceptionDialog.Ok(exception, "UxtrataRecorder", "Unhandled exception", exception.Message);
        }
